      ...
      <thead className="text-sm lg:text-base text-dark-gray">
        <tr className="border-b border-gray-200">
          <th className="text-center py-2 px-2 lg:py-4">
            <input
              type="checkbox"
              checked={items.filter(item => item.product.quantity > 0).every(item => selectedItems.includes(item._id))}
              onChange={toggleAll}
            />
          </th>
        ...
        {items.map(product => (
          <tr key={product._id} className="text-center text-sm lg:text-base border-b py-4 border-gray-200">
            ...
            <td className="text-left border-r border-gray-200">
              <div
                className="flex items-center gap-4 py-4 lg:py-6 cursor-pointer"
                onClick={() => router.push(`/products/${product.product._id}`)}
              >
                <Image
                  src={getImageUrl(product.product.image.path)}
                  unoptimized={true}
            ...
            <td className="border-r border-gray-200">
              {product.product.quantity === 0 ? (
                ...
                <div className="flex items-center border divide-x border-gray justify-center w-fit mx-auto">
                  ...
                  <button
                    onClick={() => increaseQty(product._id, product.product.quantity, product.product.buyQuantity)}
            ...
            {/* 주문 금액 */}
            <td className="border-r border-gray-200">
              {(product.product.price * getQuantity(product._id)).toLocaleString()}원
            </td>

            {/* 배송비 */}
            <td className="border-r border-gray-200">
              {product.product.shippingFees?.toLocaleString() ?? '3000'}원
            </td>
            ...
            <td>
              <button
                ...
                className="text-sm text-gray hover:text-red cursor-pointer"
              >
                삭제
              </button>
            </td>
