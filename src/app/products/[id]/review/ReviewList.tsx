...
export default function ReviewList({ reviews, loading, currentUser, expandedReviewIds, toggleExpand, openImageModal, showMyReviewsOnly, onEditClick, onDeleteClick }: ReviewListProps) {
  if (loading) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {reviews.map(review => (
        <ReviewItem key={review._id} review={review} currentUser={currentUser} expanded={expandedReviewIds.has(review._id)} toggleExpand={toggleExpand} openImageModal={openImageModal} onEditClick={onEditClick} onDeleteClick={onDeleteClick} />
      ))}
    </div>
  );
}
